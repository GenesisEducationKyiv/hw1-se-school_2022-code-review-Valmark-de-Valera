...
const loggerLevels = ['verb', 'debug', 'warn', 'error', 'info'];
...
export class LoggerService implements LoggerServicePar {
	...
	constructor(
		@Inject('Default_Name') _serviceName: string,
		@Inject('SUBSCRIBERS_LOG_RMQ_SERVICE') _subscribersRmqService: ClientProxy,
	) {
	...
	log(message: any, ...optionalParams: any[]) {
		...
		this.checkForLog('info', this.levels.rabbit)
			? this._subscribersRmqService.send({ cmd: 'consumer' }, message)
			: null;
	}
	...
	error(message: any, ...optionalParams: any[]) {
		...
		this.checkForLog('error', this.levels.rabbit)
			? this._subscribersRmqService.send({ cmd: 'consumer' }, message)
			: null;
	}
	...
	warn(message: any, ...optionalParams: any[]) {
		...
		this.checkForLog('warn', this.levels.rabbit)
			? this._subscribersRmqService.send({ cmd: 'consumer' }, message)
			: null;
	}
	...
	debug?(message: any, ...optionalParams: any[]) {
		...
		this.checkForLog('debug', this.levels.rabbit)
			? this._subscribersRmqService.send({ cmd: 'consumer' }, message)
			: null;
	}
	...
	verbose?(message: any, ...optionalParams: any[]) {
		...
		this.checkForLog('verb', this.levels.rabbit)
			? this._subscribersRmqService.send({ cmd: 'consumer' }, message)
			: null;
	}
